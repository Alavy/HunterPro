...
{
    public class ViolentZombieNpc : MonoBehaviour
    {
        [SerializeField]
        ...
        private void Awake()
        {
            ...
            var eating = new Eating(transform,m_zombieVision,Eating, WalkSpeed);
            ...
            m_stateMachine.AddTransition(eating, wandering,
               () => eating.IsAbleToGoNextState
            );

            m_stateMachine.AddAnyTransition(attacking,
                () => eating.IsOnThisState ? false : m_zombieVision.IsPlayerVisible || m_isHit);

            m_stateMachine.AddAnyTransition(eating,
                () => attacking.IsOnThisState ? false : m_zombieVision.IsDeathVisible );
